---
name: bank_check_ocr
description: 仅在用户明确提及“银行支票”、“支票号码”、“支票金额”、“支票识别”等特定词汇时触发，用于识别银行支票上的关键信息（号码、日期、大小写金额、签章等）。不适用于通用OCR或非支票类图像识别。
version: 1.0.1
author: SCNet
license: MIT
tags:
...
本技能封装了银行支票识别的 OCR 服务，通过单一接口即可调用 1 种识别能力，精准提取支票号码、出票日期、大小写金额及出票人签章等。

> **⚠️ 重要隐私与安全警告（请在使用前仔细阅读）**
>
> **数据外传**：本技能会将您提供的支票图片通过网络**传输至第三方服务商（`api.scnet.cn`）** 进行 OCR 处理，图片数据**离开您的本地环境**，并非本地处理。
>
> **数据用途**：上传的图片仅用于本次支票识别请求，服务商不会将数据用于其他任何目的。
>
...
| 参数名 | 类型 | 必填 | 描述 |
|--------|------|------|------|
| ocrType | string | 是 | 识别类型枚举。必须为以下之一：<br>• BANK_CHECK（银行支票）|
| filePath | string | 是 | 待识别图片的本地绝对路径。支持 jpg、png、pdf 等常见格式。 |

### 命令行调用示例

```bash
   python .claude/skills/bank_check_ocr/scripts/main.py BANK_CHECK /path/to/test.jpg
```
